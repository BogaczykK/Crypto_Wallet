...
def crypto_info(coin):
    request = requests.get("https://api.coingecko.com/api/v3/coins/{}".format(coin))
    return request.json()
...
class ChoiceWin(tk.Toplevel):
    def __init__(self, master=None):
        ...
        message_label = tk.Label(self, text="Select cryptocurrency")
        message_label.pack()
...
class PlotWin(tk.Toplevel):
    def __init__(self, master, time, coin):
        ...
        if time == 1:
            message = coin_name+" buy price change: 1 day"
        elif time == 7:
            message = coin_name+" buy price change: 7 days"
        else:
            message = coin_name+" buy price change: 30 days"
        high = 0
        low = math.inf
...
def upload_wallet():
    ...
    for coin, val in data.items():
        if coin not in coins:
            label["text"] = "Not supported or incorrect coin type"
            error = True
            break
        try:
            if float(val) <= 0:
            ...
            label["text"] = "Incorrect coin amount"
            error = True
            break
    if not error:
        my_wallet = data
...
if __name__ == "__main__":
    ...
    entry_amount = tk.Entry(root, width=60)
    ...
    # buttons
    # adds to wallet
    button1 = tk.Button(root, text="add to wallet", command=add_to_wallet, pady=5)
    button1.pack(fill="x")

    # Change wallet
    button7 = tk.Button(root, text="Change value in your wallet", command=change_wallet, pady=5)
    button7.pack(fill="x")

    # upload existing wallet button
    button4 = tk.Button(root, text="Upload last wallet", command=upload_wallet, pady=5)
    button4.pack(fill="x")

    # show wallet value button
    button5 = tk.Button(root, text="Wallet value in USD", command=wallet_value, pady=5)
    button5.pack(fill="x")

    # saves wallet to json button
    button3 = tk.Button(root, text="Save your wallet \n Warning!: saving will override existing data",
                        command=save_wallet, pady=5)
    button3.pack(fill="x")

    # reset wallet button
    button6 = tk.Button(root, text="Reset wallet", command=reset_wallet, pady=5)
    button6.pack(fill="x")

    # show price graph button
    button8 = tk.Button(root, text="Show price change graph", pady=5)
    button8.bind("<Button>", lambda e: ChoiceWin(root))
    button8.pack(fill="x")

    # exits program
    exit_button = tk.Button(root, text="Exit Program", command=root.quit, pady=5)
